...
proc fieldSqlType*(field: FieldMeta; backend: BackendKind): string =
  case field.kind
  of fkString:
...
proc fieldDefaultSql*(field: FieldMeta; backend: BackendKind): string =
...
proc columnDefinitionSql*(field: FieldMeta; backend: BackendKind): string =
  ...
  if backend == postgresBackend and field.primaryKey and field.autoIncrement:
    return result & "BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY"
  result.add(fieldSqlType(field, backend))
  if field.primaryKey:
